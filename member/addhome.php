...
if($gClient->getAccessToken()){
    ...
    if($rank != "Member"){
      ...
      $loai_nt = "";
      if(isset($_POST['loainhatro'])){
        foreach ($_POST['loainhatro'] as $nt) {
          $loai_nt = "".$list_nt[$nt]." ,";
        }
      }

      //Lấy giá trị POST từ form vừa submit (tennhatro vitri toado giaphong giadien gianuoc phuthu mota khuvuc home_thumbnail)
      if ($_SERVER["REQUEST_METHOD"] == "POST") {
          if(isset($_POST["tennhatro"])) { $ten_nha_tro = $_POST['tennhatro'];  }
          if(isset($_POST["vitri"])) { $vi_tri = $_POST['vitri']; }
          if(isset($_POST["toado"])) { $coord = $_POST['toado']; }
          ...
          if(isset($_POST["mota"])) { $mo_ta = $_POST['mota']; }
          if(isset($_POST["khuvuc"])) { $khu_vuc = $_POST['khuvuc']; }
          if(isset($_POST["home_thumbnail"])) { $anh_bia = $_POST['home_thumbnail']; }

          $sql2 = "INSERT INTO homes (oauth_uid, status, tennhatro, vi_tri, coordinates, gia_phong, gia_dien, gia_nuoc, phu_thu, mo_ta, loai_nha_tro, khu_vuc, anh_bia, date_modified) VALUES ('$uid', 'Pending', '$ten_nha_tro', '$vi_tri', '$coord', $gia_phong, $gia_dien, $gia_nuoc, $phu_thu, '$mo_ta', '$loai_nt', '$khu_vuc', '$anh_bia', '$date_modified')"; 
      ...
      <nav class="navbar navbar-expand-lg navbar-transparent navbar-absolute fixed-top ">
        <div class="container-fluid">
          <div class="navbar-wrapper">
            <a class="navbar-brand" href="#pablo">Thêm nhà trọ mới</a>
          </div>
            ...
            <div class="col-md-12">
              <div class="card">
                <div class="card-header card-header-primary">
                  <h4 class="card-title">Thêm nhà trọ mới</h4>
                  <p class="card-category">Nhà trọ mới sẽ được đăng tải sau khi xét duyệt!</p>
                  <p class="card-category">Đường vào nhà trọ của bạn không có trên bản đồ? <a style="color: #fff; font-weight: bold; text-transform: uppercase;" href="./add_street.php">Thêm đường</a></p>
                </div>
                <div class="card-body">
                  <form action="" method="POST" id="frm_upload">

                    <label class="font-weight-bold" for="exampleFormControlFile1">Thêm vị trí nhà trọ</label>
                   ...
                   <br>
                    ...
                    <div class="form-group">
                        <input name="tennhatro" type="text" class="form-control" id="tennhatro" placeholder="Tên nhà trọ...">
                    </div>
                    <div class="form-row">
                      <div class="form-group col-md-8">
                          <input name="vitri" type="text" class="form-control" id="vitri" placeholder="Vị trí nhà trọ...">
                      </div>
                      <div class="form-group col-md-4">
                          <input name="toado" type="text" class="form-control" id="toado" placeholder="Vị trí nhà trọ của bạn đã chọn trên bản đồ!" readonly>
                      </div>
                    </div>
                    <div class="line"></div>
                    <div class="form-row">
                        <div class="form-group col-md-3">
                          ...
                          <input name="giaphong" type="number" min="0" class="form-control" id="giaphong">
                        </div>
                        <div class="form-group col-md-3">
                          <label class="font-weight-bold" for="exampleFormControlFile1">Giá điện (VNĐ/KWh)</label>
                          <input name="giadien" type="number" min="0" class="form-control" id="giadien">
                        </div>
                        <div class="form-group col-md-3">
                          <label class="font-weight-bold" for="exampleFormControlFile1">Giá nước (VNĐ/m<sup>3</sup>)</label>
                          <input name="gianuoc" type="number" min="0" class="form-control" id="gianuoc">
                        </div>
                        <div class="form-group col-md-3">
                          <label class="font-weight-bold" for="exampleFormControlFile1">Phụ thu (wifi, giữ xe...)</label>
                          <input name="phuthu" type="number" min="0" class="form-control" id="phuthu">
                        </div>
                    </div>
                    <div class="form-group">
                        <textarea name="mota" class="form-control mota" id="mota" rows="3" placeholder="Mô tả..."></textarea>
                    </div>
                    <div class="form-row">
                        <div class="form-group col-md-6">
                            <label class="font-weight-bold" for="exampleFormControlFile1">Loại nhà trọ</label>
                            <br/>
                            <input type="checkbox" name="loainhatro[]" value="1"><label for="chk_binhthuong" style="margin-right: 10px;"> Bình thường</label>
                            <input type="checkbox" name="loainhatro[]" value="2"><label for="chk_nhatronu" style="margin-right: 10px;"> Nhà trọ nữ</label>
                            <input type="checkbox" name="loainhatro[]" value="3"><label for="chk_cogac" style="margin-right: 10px;"> Có gác</label>
                            <input type="checkbox" name="loainhatro[]" value="4"><label for="chk_khonggac"> Không gác</label>
                        </div>
                        <div class="form-group col-md-6">
                            <label class="font-weight-bold" for="exampleFormControlFile1">Khu vực (Tỉnh thành)</label>
                            <input name="khuvuc" type="text" min="0" class="form-control" id="khuvuc">
                            </div>
                        </div>
                    </div>
                    <div class="line"></div>
                    <div class="form-row" id="upload_thumbnail">
                        ...
                        <div class="form-group col-md-8 ">
                            <a target="_blank" rel="noopener noreferrer" onclick="loadPicker();">
                                <p class="btn btn-primary GDUpload">Upload to Google Drive <i class="fab fa-google-drive"></i></p>
                            </a>
  ...
  <!-- CREATE MAP -->
    ...
    <script>
        function onMapClick(e) {
            ...
            // alert("Đã chọn tại: "+lat+", "+lon+"\nBạn có thể hoàn thành hoặc chọn điểm khác!");
            md.showNotification('top','center', lat, lon);
            document.getElementById("toado").value = ""+lat+", "+lon;
        }
