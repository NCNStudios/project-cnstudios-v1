      ...
      if(isset($_REQUEST["id"]))
        {
              ...
              $datamembers = $conn->query($query_getinfo_member);

              if ($datamembers->num_rows > 0) {
                  while($row = $datamembers->fetch_assoc()) {
                      $ten_mem = $row["ten_members"];
                      $phong_mem = $row["phong"];
                      $ten_nt_mem = $row["tennhatro"];
                      $id_nt = $row["id_home"];
                  }
              }
              else {
              ...
              //End lấy thông tin mem

              //Lấy thông tin nhà trọ
              $query_getinfo_home = "SELECT * FROM homes WHERE id_home = '$id_nt'";
              $datahome = $conn->query($query_getinfo_home);

              //if6
              if ($datahome->num_rows > 0) {
                  while($home = $datahome->fetch_assoc()) {
                      $gia_dien = $home["gia_dien"];
                      $gia_nuoc = $home["gia_nuoc"];
                      $gia_phong = $home["gia_phong"];
                      $phu_thu = $home["phu_thu"];
                  }
              } 
              else {
                  echo 'No result';
              }//End lấy thông tin nhà trọ
              ...
              if ($_SERVER["REQUEST_METHOD"] == "POST") {
                ...
                $sql2 = "INSERT INTO bills (id_members, oauth_uid, ngay_bd, ngay_kt, so_dien, so_nuoc, ghi_chu, tong_tien, date_modified, ten_members, gia_phong, phu_thu, id_home) VALUES ('".$_REQUEST['id']."', '$uid', '$ngay_bd', '$ngay_kt', '$so_dien', '$so_nuoc', '$ghi_chu', '$tong_tien','$date_modified', '$ten_mem', '$gia_phong', '$phu_thu', '$id_nt')"; 
      ...
      <nav class="navbar navbar-expand-lg navbar-transparent navbar-absolute fixed-top ">
        <div class="container-fluid">
          <div class="navbar-wrapper">
            <a class="navbar-brand" href="#pablo">Thêm hóa đơn mới</a>
          </div>
            ...
            <div class="col-md-12">
              <div class="card">
                <div class="card-header card-header-primary">
                  ...
                  <p class="card-category">Phòng <?php echo $phong_mem ?> <i class="fas fa-home"></i> <?php echo $ten_nt_mem ?></p>
                </div>
                <div class="card-body">
                  <form action="" method="POST">
                    <div class="row">
                      <div class="col-md-6">
                        <div class="form-group">
                          ...
                          <input type="text" class="form-control" value="<?php echo $ten_mem ?>" disabled>
                        </div>
                      </div>
                      <div class="col-md-3">
                        <div class="form-group">
                          <label class="bmd-label-floating">Phòng</label>
                          <input type="text" class="form-control" value="<?php echo $phong_mem ?>" disabled>
                        </div>
                      </div>
                      <div class="col-md-3">
                        <div class="form-group">
                          <label class="bmd-label-floating">Thuộc nhà trọ</label>
                          <input type="text" class="form-control" value="<?php echo $ten_nt_mem ?>" disabled>
                        </div>
                      </div>
                    </div>
                    <div class="row">
                      <div class="col-md-6">
                        <div class="form-group">
                          ...
                          <input name="ngayBD" type="text" class="form-control" id="start_datepicker">
                        </div>
                      </div>
                      <div class="col-md-6">
                        <div class="form-group">
                          <label>Ngày kết thúc</label>
                          <input name="ngayKT" type="text" id="end_datepicker" class="form-control">
                        </div>
                      </div>
                    </div>
                    <div class="row">
                      <div class="col-md-2">
                        <div class="form-group">
                          ...
                          <input name="soDien" id="soDien" type="number" min="0" class="form-control" >
                        </div>
                      </div>
                      <div class="col-md-2">
                        <div class="form-group">
                          <label class="bmd-label-floating">Giá điện (Đồng/KWh)</label>
                          <input type="number" id="giaDien" min="0" class="form-control" disabled value="<?php echo $gia_dien ?>">
                        </div>
                      </div>
                      <div class="col-md-2">
                        <div class="form-group">
                          <label class="bmd-label-floating">Số nước</label>
                          <input name="soNuoc" id="soNuoc" type="number" min="0" class="form-control">
                        </div>
                      </div>
                      <div class="col-md-2">
                        <div class="form-group">
                          <label class="bmd-label-floating">Giá nước (Đồng/m<sup>3</sup>)</label>
                          <input type="number" id="giaNuoc" min="0" class="form-control" disabled value="<?php echo $gia_nuoc ?>">
                        </div>
                      </div>
                      <div class="col-md-2">
                        <div class="form-group">
                          <label class="bmd-label-floating">Tiền phòng</label>
                          <input  type="number" id="tienPhong" min="0" class="form-control" disabled value="<?php echo $gia_phong ?>">
                        </div>
                      </div>
                      <div class="col-md-2">
                        <div class="form-group">
                          <label class="bmd-label-floating">Phụ thu</label>
                          <input type="number" id="phuThu" min="0" class="form-control" disabled value="<?php echo $phu_thu ?>">
                        </div>
                      </div>
                    </div>
                    <div class="row">
                      <div class="col-md-12">
                        <div class="form-group">
                          ...
                          <input name="tongTien" id="tongTien" type="text" class="form-control" id="start_datepicker" readonly>
                        </div>
                      </div>
                    </div>
                    <div class="row">
                    ...
                    <button type="submit" class="btn btn-primary pull-right">Tạo hóa đơn</button>
